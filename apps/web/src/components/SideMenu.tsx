...
export default function SideMenu({ isOpen, onClose }: SideMenuProps) {
  ...
  const menuItems = [
    ...
    { divider: true },
    { icon: Info, label: t('aboutApp'), subtitle: 'Messenger Test v1.0', onClick: () => changeView('about') },
  ];
        ...
        <div className="px-5 py-3">
          ...
          <div className="flex items-center gap-4 px-3 py-3 rounded-xl bg-surface-tertiary/50">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm text-zinc-200">Messenger Test</p>
    ...
    <motion.div key="about" className="flex flex-col h-full" initial={{ x: 100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 100, opacity: 0 }} transition={{ duration: 0.2 }}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
        ...
        <div className="text-xs text-zinc-500 space-y-1">
          ...
          <p>{t('onPrivacy')}</p>
          <p className="mt-4 text-zinc-600">© 2026 Messenger Test Team</p>
        </div>
      </div>
    </motion.div>
  );
